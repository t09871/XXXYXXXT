# discoverBirdbillState-v0.2.ps1 | v0.2 | 2026-07-07 PDT | Read-only current-state discovery for D:\birdbill

$ErrorActionPreference = "Continue"

$scriptVersion = "v0.2"
$scriptName = "discoverBirdbillState-v0.2.ps1"
$root = "D:\birdbill"
...
function Get-FolderSummaryRows {
    param(
        [Parameter(Mandatory=$true)][string]$FolderPath,
        [int]$Depth = 1
    )
...
function Get-SourceConfigInventory {
    ...
    Get-ChildItem -LiteralPath $FolderPath -File -Recurse -Force -ErrorAction SilentlyContinue |
        ...
        ForEach-Object {
            ...
            $likelyRole = "root-or-uncategorized"
            if ($relative -match '^app\\') {
                $likelyRole = "app"
            } elseif ($relative -match '^debug\\') {
                $likelyRole = "debug-script-or-debug-note"
            } elseif ($relative -match '^modules\\') {
                $likelyRole = "module-or-module-config"
            } elseif ($relative -match '^output\\debug\\') {
                $likelyRole = "generated-debug-artifact"
            } elseif ($relative -match '^output\\cache\\') {
                $likelyRole = "generated-cache-artifact"
            } elseif ($relative -match '^output\\') {
                $likelyRole = "generated-output-or-derived-state"
            }
...
function Write-LikelyModuleClassification {
    ...
    $interestingPaths = @(
        ...
        "modules\megadetector\megadetector-env\Scripts\python.exe",
        "modules\megadetector\models\MDV6b-yolov9-c.pt",
        "modules\dlc",
        "modules\dlc\billtip",
        "modules\dlc\billtip\billtip-HB-2026-06-30",
        "modules\dlc\billtip\billtip-HB-2026-06-30\config.yaml",
        "modules\speciesnet",
        "modules\speciesnet\speciesnet-env\Scripts\python.exe",
        "modules\mmpose",
        "output",
        "output\database",
        "output\debug",
    ...
    foreach ($rel in $interestingPaths) {
        ...
        if ($exists) {
            ...
            if ($rel -eq "app\SmartFrameSampler.py") {
                ...
                $classification = "module-area-or-module-artifact"
...
try {
    ...
    "discoverBirdbillState-v0.2.ps1 | v0.2 | 2026-07-07 PDT | Read-only current-state discovery for D:\birdbill" | Set-Content -LiteralPath $statusPath -Encoding UTF8
